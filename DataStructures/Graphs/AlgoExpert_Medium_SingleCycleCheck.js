...
function hasSingleCycle(array) {
    // Write your code here.
      ...
      while(numVisited < array.length){
          if(numVisited > 0 && current === 0){
              return false;
          }
          numVisited+=1;
          current = newCurrent(current, array);
      }
      return current === 0;
  }
  
  function newCurrent(current, array){
      var newPointer = (current + array[current]) % array.length;
      if(newPointer >= 0){
          return newPointer;	
      }
      return newPointer + array.length;
  }
